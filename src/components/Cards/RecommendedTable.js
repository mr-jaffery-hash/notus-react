...
export default function RecommendedTable({ color, jobs }) {
  ...
  useEffect(() => {
    const fetchDataFiller = async () =>{
      const a = await fetchData(localStorage.getItem("email"));
    }
    fetchDataFiller()
    // const fetchDataAndFilter = async () => {
    //   //const fetchedData = await fetchData(localStorage.getItem("email"));
    //   //console.log('return', similarJobs);
    //   setSimilarJobs(await fetchData(localStorage.getItem("email")));
    //   let filterData = [];
      ...
      >
        ...
        <div className="block w-full overflow-x-auto">
          ...
          <table className="items-center w-full bg-transparent border-collapse">
            ...
            <tbody>
              {jobs && jobs.map((item) => { 
                if(similarJobs.includes(item.title)===true){
                  return(
                    <tr
                      onMouseEnter={(e) => {
